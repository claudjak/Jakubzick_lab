...
Provider: Anthropic when ANTHROPIC_API_KEY is set, otherwise OpenAI (OPENAI_API_KEY). Override with JLAB_LLM_PROVIDER / JLAB_LLM_MODEL.
"""
import os
import time
...
def get_classifier():
    global _clf
    if _clf is None:
        provider = os.environ.get("JLAB_LLM_PROVIDER") or ("anthropic" if (os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN")) else "openai")
        model = os.environ.get("JLAB_LLM_MODEL") or {"anthropic": "claude-sonnet-5", "openai": "gpt-5-mini"}[provider]
        _clf = Classifier(provider=provider, model=model, effort=os.environ.get("JLAB_LLM_EFFORT", "medium"))
    return _clf
